...
class ProjectTask(models.Model):
    ...
    def action_fsm_validate(self, stop_running_timers=False):
        ...
        # Validar imágenes mínimas ANTES de cerrar
        for task in self:
    ...
    """Funciones para que solo vean sus propios registros los colocadores
        Revisar con vista kanban
    """
    
    # @api.model
    # def _name_search(self, name, domain=None, operator='ilike', limit=None, order=None):
    #     """Restrict task search based on user permissions"""
    #     user = self.env.user
    ...
    #     # Only apply restriction if user has the custom permission
    #     if user.has_group('fs_m2_cost.group_fsm_task_user_own'):
    #         list_user_ids = self.env.user.ids
    #         # Domain to show only tasks where user is assigned
    #         domain_own_tasks = [('user_ids', 'in', list_user_ids)]
    #         # Combine domains using & operator
    #         domain = odoo_fields.Domain(domain or []) & odoo_fields.Domain(domain_own_tasks)
        
    #     return super()._name_search(name, domain, operator, limit, order)
    
    # @api.model
    # def web_search_read(self, domain, specification, offset=0, limit=None, order=None, count_limit=None):
    #     """Restrict task list view based on user permissions"""
    #     user = self.env.user
    ...
    #     """Restrict task search_read based on user permissions"""
    #     user = self.env.user
        
    #     # FOR SUPER USER WE DO NOTHING
    #     if self.env.su:
    #         return super().search_read(domain, fields, offset, limit, order, **kwargs)
    ...
    #         # Domain to show only tasks where user is assigned
    #         domain_own_tasks = [('user_ids', 'in', list_user_ids)]
    #         # Combine domains using & operator - use odoo_fields to avoid conflict with parameter
    #         domain = odoo_fields.Domain(domain or []) & odoo_fields.Domain(domain_own_tasks)
