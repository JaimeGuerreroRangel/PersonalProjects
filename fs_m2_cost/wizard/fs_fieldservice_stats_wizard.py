...
class FsFieldserviceStatsWizard(models.TransientModel):
    ...
    measurement_with_so_count = fields.Integer(
        string="Tomas de medida con OV", readonly=True
    )
    measurement_with_so_by_company = fields.Text(
        string="Desglose con OV por compañía", readonly=True
    )
    
    # Tomas de medida sin OV
    measurement_without_so_count = fields.Integer(
        string="Tomas de medida sin OV", readonly=True
    )
    measurement_without_so_by_company = fields.Text(
        string="Desglose sin OV por compañía", readonly=True
    )
    
    # Total de tomas de medida (calculado)
    measurement_count = fields.Integer(
        string="Tomas de medida totales", readonly=True, compute="_compute_measurement_total", store=True
    )
    
    # Campo legacy mantenido para compatibilidad
    measurement_sale_count = fields.Integer(
        string="Tomas de medida con venta (legacy)", readonly=True
    )
    ...
    measurement_with_so_task_ids = fields.Many2many(
        "project.task",
        "fs_stats_measurement_with_so_task_rel",
        "wizard_id",
        "task_id",
        string="Tomas de medida con OV",
        readonly=True,
    )
    measurement_without_so_task_ids = fields.Many2many(
        "project.task",
        "fs_stats_measurement_without_so_task_rel",
        "wizard_id",
        "task_id",
        string="Tomas de medida sin OV",
        readonly=True,
    )
    measurement_sale_task_ids = fields.Many2many(
        ...
        "wizard_id",
        "task_id",
        string="Tomas de medida con venta (tareas)",
        readonly=True,
    )
    ...
    installation_on_time_ids = fields.Many2many(
        ...
        "wizard_id",
        "task_id",
        string="Colocaciones a tiempo (tareas)",
        readonly=True,
    )
    installation_late_ids = fields.Many2many(
        ...
        "wizard_id",
        "task_id",
        string="Colocaciones tarde (tareas)",
        readonly=True,
    )
    installation_no_show_ids = fields.Many2many(
        ...
        "wizard_id",
        "task_id",
        string="Colocaciones sin llegada (tareas)",
        readonly=True,
    )
    ...
    @api.depends('measurement_with_so_count', 'measurement_without_so_count')
    def _compute_measurement_total(self):
        for record in self:
            record.measurement_count = record.measurement_with_so_count + record.measurement_without_so_count
    ...
    def action_compute(self):
        ...
        if not tasks:
            self.write(
                {
                    ...
                    "on_time_rate": 0.0,
                    "measurement_with_so_count": 0,
                    "measurement_without_so_count": 0,
                    "measurement_with_so_by_company": "",
                    "measurement_without_so_by_company": "",
            ...
            self.measurement_with_so_task_ids = [(5, 0, 0)]
            self.measurement_without_so_task_ids = [(5, 0, 0)]
        ...
        # Nuevos contadores para tomas de medida
        meas_with_so = 0
        meas_without_so = 0
        meas_with_so_by_company = {}  # company_id -> count
        meas_without_so_by_company = {}  # company_id -> count
        ...
        meas_tasks = set()
        meas_with_so_tasks = set()
        meas_without_so_tasks = set()
        meas_sale_tasks = set()
        inst_tasks = set()
        ...
        for task in tasks:
            ...
            # Tomas de medida - NUEVA LÓGICA
            if is_measure:
                meas_tasks.add(task.id)
                
                # Obtener compañía de la tarea
                company_name = task.company_id.name if task.company_id else "Sin compañía"
                
                # Separar por si tiene o no orden de venta
                if sale:
                    meas_with_so += 1
                    meas_with_so_tasks.add(task.id)
                    meas_with_so_by_company[company_name] = meas_with_so_by_company.get(company_name, 0) + 1
                    
                    # Legacy: venta confirmada
                    if sale.state in ("sale", "done"):
                        meas_sale += 1
                        meas_sale_tasks.add(task.id)
                else:
                    meas_without_so += 1
                    meas_without_so_tasks.add(task.id)
                    meas_without_so_by_company[company_name] = meas_without_so_by_company.get(company_name, 0) + 1
        ...
        # Formatear desglose por compañía
        with_so_text = "\n".join([f"{company}: {count}" for company, count in sorted(meas_with_so_by_company.items())])
        without_so_text = "\n".join([f"{company}: {count}" for company, count in sorted(meas_without_so_by_company.items())])
        ...
        self.write(
            {
                ...
                "measurement_with_so_count": meas_with_so,
                "measurement_without_so_count": meas_without_so,
                "measurement_with_so_by_company": with_so_text,
                "measurement_without_so_by_company": without_so_text,
        ...
        self.measurement_with_so_task_ids = [(6, 0, list(meas_with_so_tasks))]
        self.measurement_without_so_task_ids = [(6, 0, list(meas_without_so_tasks))]
    ...
    def action_open_measurements_with_so(self):
        return self._open_tasks_action(
            _("Tomas de medida con OV"), self.measurement_with_so_task_ids
        )

    def action_open_measurements_without_so(self):
        return self._open_tasks_action(
            _("Tomas de medida sin OV"), self.measurement_without_so_task_ids
        )
